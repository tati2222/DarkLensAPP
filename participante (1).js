...
async function analizarMicroexpresiones() {
    ...
    try {
        ...
        // 3) Mostrar resultados y botón para ver análisis completo
...
// ========================================
// FUNCIONES AGREGADAS PARA REDIRECCIÓN
// ========================================
...
function mostrarResultados(datos) {
    ...
    html += `
        ...
        <button onclick="guardarYRedirigir()" class="btn-primary" style="margin-top: 20px;">
...
function guardarYRedirigir() {
    // Asegurar que todos los datos estén guardados
    const datosPersonales = JSON.parse(sessionStorage.getItem('datos_personales') || '{}');
    const resultadosSD3 = JSON.parse(sessionStorage.getItem('resultadosSD3') || '{}');
    const resultadosMicro = JSON.parse(sessionStorage.getItem('resultadosMicro') || '{}');
    
    // Verificar que tenemos los datos necesarios
    if (!datosPersonales.nombre || !resultadosSD3.mach || !resultadosMicro.emociones) {
...
// Inicializar tracking de tiempos cuando se generan los items
const originalGenerarItemsTest = generarItemsTest;
generarItemsTest = function() {
    testInicioTimestamp = Date.now();
    tiemposRespuesta = {};
    tiempoInicioItem = {};
    originalGenerarItemsTest();
